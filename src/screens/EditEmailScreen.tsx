...
import { getFundraiserAccount, updateFundraiserAccount, generateAndStoreEmailOTP, verifyEmailOTP } from '../utils/donations';

const { width: W } = Dimensions.get('window');
const sc = (v: number) => Math.round(v * (W / 390));

export default function EditEmailScreen({ navigation, theme }: any) {
  ...
  const handleSendCode = async () => {
    ...
    const code = await generateAndStoreEmailOTP(newEmail);
    setGeneratedCode(code);
    setSentOTP(true);
    Alert.alert('Email Verification', `Your verification code is: ${code}\n\n(Enter this code to verify your new email)`, [{ text: 'OK' }]);
  };
  ...
  const handleVerify = async () => {
    ...
    await updateFundraiserAccount({ email: newEmail, emailVerified: true });
  ...
  return (
    ...
    >
      ...
      <View style={[styles.topBar, { paddingTop: insets.top }]}>
        ...
        <Text style={[styles.topTitle, { color: colors.onSurface }]}>Edit Email</Text>
      ...
      <View style={styles.content}>
        ...
        <Text style={[styles.title, { color: colors.onSurface }]}>
          {sentOTP ? 'Verify New Email' : 'Change Email'}
        </Text>
        <Text style={[styles.subtitle, { color: colors.onSurfaceVariant }]}>
          {sentOTP
            ? `We sent a 6-digit code to ${newEmail}`
            : 'Enter your new email address. We will send a verification code to confirm it belongs to you.'}
        </Text>

        {/* Email Input */}
        {!sentOTP && (
          <View style={styles.form}>
            <Text style={[styles.fieldLabel, { color: colors.onSurfaceVariant }]}>Current Email</Text>
            <View style={[styles.readonlyWrap, { backgroundColor: colors.surfaceContainerHigh }]}>
              <Ionicons name="mail-outline" size={sc(16)} color={colors.onSurfaceVariant} />
              <Text style={[styles.readonlyText, { color: colors.onSurfaceVariant }]}>{originalEmail}</Text>
            </View>

            <Text style={[styles.fieldLabel, { color: colors.onSurfaceVariant, marginTop: sc(16) }]}>New Email</Text>
            ...
            <TouchableOpacity
              style={[styles.ctaWrap, { opacity: newEmail.trim() && newEmail !== originalEmail ? 1 : 0.4 }]}
              onPress={handleSendCode}
              disabled={!newEmail.trim() || newEmail === originalEmail}
              activeOpacity={0.85}
            >
